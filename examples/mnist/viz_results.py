...
# Double check plot data
# Extend to plots comparing methods...
# Extend to multiple data sets per file
# Is code below correct?  Can you put the thing in a function or make it better...?  Maybe not now...

colors = ['-k', '-m', '-b', '-r', '-c', '-g', '-y', '-k', '-m', '-b', '-r', '-c', '-g', '-y', '-k', '-m', '-b', '-r', '-c', '-g', '-y']
colors2 = ['--k', '--m', '--b', '--r', '--c', '--g', '--y', '--k', '--m', '--b', '--r', '--c', '--g', '--y', '--k', '--m', '--b', '--r', '--c', '--g', '--y']

glob_losses1, glob_train_losses1, glob_accur1, glob_num_correct1 = grab_losses_acc_and_NI_MGOPT_transitions('comparison_TB_NI.out1')
glob_losses2, glob_train_losses2, glob_accur2, glob_num_correct2 = grab_losses_acc_and_NI_MGOPT_transitions('comparison_TB_MGOpt.out1')
glob_losses3, glob_train_losses3, glob_accur3, glob_num_correct3 = grab_losses_acc_and_NI_MGOPT_transitions('comparison_TB_MGOpt_LocalRelax.out1')
