...
from model.transformer_decoder_residual_layer import TransformerDecoderResidualLayer_selfonly, TransformerDecoderResidualLayer_crossmlponly
...
class StepLayer_dec_selfonly(nn.Module):
  def __init__(self, batch_size, source_length, target_length, device, **kwargs):
    super().__init__()
    torch.manual_seed(0)
    self.F = TransformerDecoderResidualLayer_selfonly(**kwargs, max_length=target_length, device=device)
  ...
  def forward(self, x):
    ...
    x = torch.stack((self.zeros_tensor[:mem.shape[0]], y))
    t1 = time.time()
    print(f'Fwd time decoder self-attn only layer: {t1 - t0} seconds')

    return x

class StepLayer_dec_crossmlponly(nn.Module):
  def __init__(self, batch_size, source_length, target_length, device, **kwargs):
    super().__init__()
    torch.manual_seed(0)
    self.F = TransformerDecoderResidualLayer_crossmlponly(**kwargs, max_length=target_length, device=device)
  ...
  def forward(self, x):
    ...
    x = torch.stack((self.zeros_tensor[:mem.shape[0]], y))
    t1 = time.time()
    print(f'Fwd time decoder x+mlp layer: {t1 - t0} seconds')
...
class ParallelNet(nn.Module):
  def __init__(self, model_dimension, num_heads, dim_ff, tokenizer, pad_id, bos_id, eos_id, device, batch_size, source_length, target_length,
    ...
    step_layer_dec_selfonly = lambda: StepLayer_dec_selfonly(d=model_dimension, num_heads=num_heads, dim_ff=dim_ff, batch_size=batch_size, source_length=source_length, target_length=target_length, device=device)
    step_layer_dec_crossmlponly = lambda: StepLayer_dec_crossmlponly(d=model_dimension, num_heads=num_heads, dim_ff=dim_ff, batch_size=batch_size, source_length=source_length, target_length=target_length, device=device)
    ...
    # Construct layers explicitly
    layers = []
    num_steps = []
    # First a layer of encoders
    for i in range(local_steps * numprocs):
      layers.append(step_layer_enc)
      num_steps.append(1)
    print(layers)
    # Next alternate the decoder stuff
    for i in range(local_steps * numprocs):
