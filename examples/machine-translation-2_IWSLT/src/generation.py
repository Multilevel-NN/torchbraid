...
def generate_beam_search(
  ...
  src = src.repeat_interleave(num_beams, dim=0)                           #/    src: [b*B, Ls]
  memory, tgt, encoder_masks, expanded_batch_size, finished_sentences = \
  ...
  for step in range(max_new_tokens):
    ...
    if do_sample:
      new_token_scores = topk_filtering(new_token_scores, top_k)                #/ [b*B,   m]
      new_token_scores = topp_filtering(new_token_scores, top_p)                #/ [b*B,   m]
    ...
    for batch_idx in range(batch_size):
      if done[batch_idx]:
        assert len(generated_hypotheses) >= num_beams
...
def prepare_decoding(model, src):
  encoder_masks = model.get_encoder_masks(src)
...
def topk_filtering(new_token_scores, top_k):
  # print('Warning: top_k filtering not implemented. Ingored.')
  return new_token_scores

def topp_filtering(new_token_scores, top_p):
  # print('Warning: top_k filtering not implemented. Ingored.')
  return new_token_scores
